...
**Не строгое** равенство сравнивает значения. **Строгое** дополнительно сравнивает их тип данных
...
3. Переменная объявленная черезе **const** становится константой и ее невозможно переопределить, но это не точно _(см. Дополнительный вопрос)_.
...
### 4. `Для чего используется оператор !! (двойное отрицание) в JS?`
<details><summary><i><h7>click to expand!</h7></i></summary>
...
### 5. `Что такое NaN? Как определить, что значение равно NaN?`
<details><summary><i><h7>click to expand!</h7></i></summary>
...
Особенностью **NaN** можно назвать то, что это значение не равно любому другому, включая себя. Стандартные методы сравнения не работают. Что бы проверить является ли значение _не числом_ используют **isNaN(то что проверяем)**
 ...
 * **Блочная _(появилась начиная с ES6)_** - для переменных **let** и **const**. Такая область видимости находится внутри фигурных скобок _(блока)_, даже если это **if** / **else**. Переменные объявленные через **var** на такую область не реагируют
...
Но если кто-то сделал ошибку в своем 💩коде или в следствии запутанности 💩кода может произойти ситуация, когда значение **_this_** функции изменяется - это называется **потеря контекста**. Чтобы такого не было и чтобы строго определить контекст вызова юзают `.call()`, `.apply()` или `.bind()`

Каждый из этих методов первым аргументом принемает контект вызова. Разница между `.call()` и `.apply()` в том, как передаются аргументы внутри функции _(в .call() через запятую пишут, в .apply() в виде массива)_.

Отличие `.call()` и `.apply()` от `.bind()` в том, что `.call()` и `.apply()` вызываются на месте _(в момент, когда их применяют. Результат получаем сразу же)_, в то время как `.bind()` создает функцию обертку, которую можно присвоить в переменную и затем вызвать в любом нужном месте приложения.
...
### 8. `function declaration vs. function expression. В чем разница?`
<details><summary><i><h7>click to expand!</h7></i></summary>
...
### 9. `Что такое чистая функция (Pure function)?`
<details><summary><i><h7>click to expand!</h7></i></summary>
...
### 10. `Что такое функции высшего порядка (Higher Order Function)?`
<details><summary><i><h7>click to expand!</h7></i></summary>
...
Не надо перечислять все... просто знать часть хотя бы _(p.s. в примере не все перечислены)_
```JavaScript
...
#### Дополнительный вопрос: Разница между .forEach() и .map() ?
...
#### 4. **Поиск минимального и максимального значения в массиве?**
<details><summary><i><h7>click to expand!</h7></i></summary>

Нужно вернуть массив. Причем если в массиве 1 элемент, то вернуться должно 2 элемента
```JavaScript
// Пример:
...
// ES6 lvl
const accum = string => string.toUpperCase()
                              .split('')
